		...
		<thead>
			<tr class="bg-gray-100 border-b-2 border-gray-400">
				...
				<td class="hidden sm:block">
					<input id="filter" v-model="filter" class="bg-gray-100 focus:outline-none border-b border-gray-400 py-2 px-4 block w-full appearance-none leading-normal" placeholder="Buscar..." type="text" />
				</td>
			</tr>
		</thead>
		<tbody>
	...
	export default {
		...
		computed: {
			filteredAssets() {
				...
				return this.assets
					.filter((asset) => asset.name.toLowerCase().includes(this.filter.toLowerCase()) || asset.symbol.toLowerCase().includes(this.filter.toLowerCase()))
					.sort((a, b) => {
